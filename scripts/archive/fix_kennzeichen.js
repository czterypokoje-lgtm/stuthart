...
export default function KennzeichenForm() {
  ...
  const handleWhatsappSubmit = async (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // Server-side lead capture silently in background
    try {
  ...
  const handleCallbackSubmit = async () => {
    ...
    if (!phone) {
      // Just focus or let html5 validation handle it if we make it a required field, 
      // but since it's custom, we will alert inline or just return if not filled.
      // But we will disable the button if phone is empty.
      return;
    }

    setIsSubmitting(true);
    try {
      const res = await fetch("https://formsubmit.co/ajax/info@fc-key.de", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Accept": "application/json" },
        body: JSON.stringify({
    ...
    return (
      <div className={styles.container} style={{ textAlign: 'center', padding: '3rem 1.5rem' }}>
        ...
        <p className={styles.subtitle} style={{ marginBottom: 0 }}>
          Wir haben Ihre Rückrufanfrage erhalten und melden uns in Kürze telefonisch bei Ihnen.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={styles.container} id="offerte-form">
      ...
      <p className={styles.subtitle}>
        Geben Sie Ihre Daten ein und erhalten Sie sofort den genauen Preis für Ihr Auto per WhatsApp oder Telefon.
      </p>
      ...
      <div className={styles.formGroup}>
        ...
        <div className={styles.inputGroup}>
          ...
          <div 
            className={styles.licensePlateWrapper} 
          ...
          {error && (
            <div id="kenteken-error" style={{ color: '#ef4444', fontSize: '0.85rem', fontWeight: 600, marginTop: '0.25rem' }}>
              Bitte geben Sie Ihr Kennzeichen ein, um ein Preisangebot zu erhalten.
            </div>
          )}
        </div>
        ...
        {/* Dynamic Callback Section */}
        {showCallback && (
          <div className={styles.inputGroup} style={{ animation: 'fadeIn 0.3s ease-out' }}>
            <label className={styles.label}>4. Wie erreichen wir Sie?</label>
            <input
              type="tel"
              className={styles.cityInput}
              placeholder="017X-XXXXXXX"
              value={phone}
            ...
            <button 
              ...
              disabled={isSubmitting || !kenteken || !phone}
              className={styles.btnSubmit}
              style={{ marginTop: '0.5rem', background: 'linear-gradient(135deg, #0f172a, #334155)' }}
            >
              {isSubmitting ? 'Wird gesendet...' : '📞 Jetzt Rückruf anfordern'}
            </button>
          </div>
        )}
        ...
        {!showCallback && (
          <div className={styles.actions}>
            ...
            <span style={{ color: '#475569', fontSize: '0.95rem' }}>oder</span>
            ...
            <button 
              ...
              style={{ background: 'transparent', border: '1px solid #cbd5e1', padding: '0.75rem 1.5rem', borderRadius: '10px', color: '#334155', fontWeight: 600, cursor: 'pointer', fontSize: '0.95rem', width: '100%', transition: 'background 0.2s' }}
            >
              Rückruf anfordern (ohne WhatsApp)
            </button>
          </div>
        )}
      </div>
