...
// Replace CLEAN 3-COLUMN MODEL LIST
const startList = `        {/* ── CLEAN 3-COLUMN MODEL LIST (NEW DESIGN) ── */}`;
const endList = `        {/* ── KEY PHOTOS SECTION (ANTI-THIN CONTENT) ── */}`;
const listIndex = content.indexOf(startList);
const listEndIndex = content.indexOf(endList);
...
if (listIndex !== -1 && listEndIndex !== -1) {
  content = content.substring(0, listIndex) + newTimpsonSection + content.substring(listEndIndex);
}

// Replace BrandModelsList with WHY CHOOSE US
const brandModelsLine = \`        {/* ── MODELS LIST ── */}
        <BrandModelsList brand={brand} />\`;
...
content = content.replace(brandModelsLine, newWhyChooseUs);

// Remove unused BrandModelsList import
content = content.replace(/import BrandModelsList.*?\n/, '');
